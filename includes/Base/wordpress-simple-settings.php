 ...
 **/
abstract class Cartimize_WordPress_SimpleSettings {
	var $settings = array();
	var $prefix;
	var $delimeter;
	var $suffix;
	...
	public function __construct() {
		...
		// Set a default suffix
		if ( empty($this->suffix) ) $this->suffix = "_settings";
	...
	public function save_settings() {
		if( isset($_REQUEST["{$this->prefix}_setting"]) && check_admin_referer("save_{$this->prefix}_settings","{$this->prefix}_save") ) {
			...
			do_action("{$this->prefix}{$this->suffix}_saved");
		}
	}
	...
	public function get_settings_obj () {
		return get_option("{$this->prefix}{$this->suffix}", false);
	}
	...
	public function set_settings_obj ( $newobj ) {
		return update_option("{$this->prefix}{$this->suffix}", $newobj);
	}
}
